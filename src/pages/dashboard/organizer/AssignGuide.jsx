...
import { fetchGuides, assignGuideToTour } from "../../../api/tourApi";
import React from "react";
import { useUIStore } from "../../../store/useUIStore";

export default function AssignGuide({ tourId }) {
  const queryClient = useQueryClient();
  const showSnackbar = useUIStore((state) => state.showSnackbar);

  // 1️⃣ Fetch all available guides
  const { data: guides, isLoading, isError, error } = useQuery(
    ["guides"],
    fetchGuides,
    {
      staleTime: 5 * 60 * 1000, // cache for 5 mins
      onError: (err) => showSnackbar(err.message || "Failed to load guides", "error"),
    }
  );

  // 2️⃣ Mutation: assign guide
  const mutation = useMutation(assignGuideToTour, {
    onSuccess: () => {
      showSnackbar("Guide assigned successfully!", "success");
      queryClient.invalidateQueries(["tour", tourId]); // refresh tour data
    },
    onError: (err) =>
      showSnackbar(err.message || "Failed to assign guide", "error"),
  });

  // 3️⃣ Loading / error states
  if (isLoading) return <p>Loading guides...</p>;
  if (isError) return <p>Error loading guides: {error.message}</p>;
  if (!guides || guides.length === 0) return <p>No guides available.</p>;

  return (
    <div className="space-y-2">
      <h3 className="text-lg font-semibold">Assign Guide</h3>
      <div className="flex flex-col gap-2">
        {guides.map((guide) => (
          <button
            key={guide.id}
            onClick={() => mutation.mutate({ tourId, guideId: guide.id })}
            disabled={mutation.isLoading}
            className="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600 disabled:bg-gray-400"
          >
            {mutation.isLoading ? "Assigning..." : guide.name}
          </button>
        ))}
      </div>
    </div>
